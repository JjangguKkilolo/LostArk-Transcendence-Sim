...
현재 `1턴 Monte Carlo`와 휴리스틱 후속 롤아웃 단계까지 구현했다. 고정 샘플 수를
사용하면 같은 상태와 AI 시드에서 같은 결과를 재현한다. 시간 한도를 사용해 일부
샘플만 완료한 결과는 실행 환경의 속도에 따라 표본 수가 달라질 수 있으므로
`completedSamples`를 함께 표시해야 한다. 공식 초파고의 계산 결과를 재현한다고
주장하지 않는다.
